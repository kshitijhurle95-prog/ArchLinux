...
if status is-interactive
  ...
  # Show fastfetch only on the first terminal (does not run when multiple terminals/tabs are open).
  set -l is_first 1
  if set -q KITTY_WINDOW_ID; and test "$KITTY_WINDOW_ID" -gt 1
    set is_first 0
  else if command -v hyprctl >/dev/null 2>&1; and set -q HYPRLAND_INSTANCE_SIGNATURE
    set -l term_count (hyprctl clients -j 2>/dev/null | grep -i -E '"class":\s*"(kitty|alacritty|ghostty|foot|wezterm|org\.wezfurlong\.wezterm|gnome-terminal|xterm|rio|konsole|terminator|xfce4-terminal|blackbox)"' | count)
    test -z "$term_count"; and set term_count 0
