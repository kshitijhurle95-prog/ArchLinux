...
PanelWindow {
    ...
    // The app quit or dropped its item while the card was open.
    onCurrentItemChanged: if (root.trayMenuVisible && !currentItem) root.trayMenuVisible = false
    ...
    function activate(id, customAction) {
        if (customAction) {
            var item = currentItem
            root.trayMenuVisible = false
            if (!item) return
            if (customAction === "focus") {
                if (item.windowAddress) {
                    Hyprland.dispatch('hl.dsp.focus({ window = "address:' + item.windowAddress + '" })')
                    Quickshell.execDetached(["hyprctl", "dispatch", "focuswindow", "address:" + item.windowAddress])
                } else if (item.service) {
                    Tray.activate(item.service)
                }
            } else if (customAction === "pin") {
                root.trayToggleHide(item)
            } else if (customAction === "close") {
                if (item.windowAddress) {
                    Hyprland.dispatch('hl.dsp.window.close({ window = "address:' + item.windowAddress + '" })')
                    Quickshell.execDetached(["hyprctl", "dispatch", "closewindow", "address:" + item.windowAddress])
                } else if (item.service) {
                    Tray.contextMenu(item.service)
                }
            }
            return
        }
        Tray.menuEvent(trayMenu.service, id)
        root.trayMenuVisible = false
    }
    ...
    function rowsFor(node, depth, out) {
        ...
        for (var i = 0; i < kids.length; i++) {
            ...
            out.push({ node: c, depth: depth, sep: sep, hasKids: hasKids, customAction: "" })
            if (hasKids && trayMenu.isExpanded(c.id)) trayMenu.rowsFor(c, depth + 1, out)
        }
        return out
    }
    readonly property var rows: {
        if (!root.trayMenuVisible) return []
        if (menu) {
            return rowsFor(menu, 0, [])
        }
        if (currentItem) {
            var out = []
            var isPinned = root.trayPinned.indexOf(currentItem.service) >= 0

            out.push({
                node: {
                    id: "focus_window",
                    label: I18n.tr("Open Window"),
            ...
            out.push({
                node: {
                    id: "pin_app",
                    label: isPinned ? I18n.tr("Unpin from Bar") : I18n.tr("Pin to Bar"),
            ...
            out.push({
                node: { id: "sep_custom", type: "separator" },
                depth: 0,
                sep: true,
                hasKids: false,
                customAction: ""
            ...
            out.push({
                node: {
                    id: "close_window",
                    label: currentItem.isWindow ? I18n.tr("Close Window") : I18n.tr("Quit"),
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                if (entry.hasKids)
                                    ...
                                    trayMenu.activate(entry.node.id, modelData.customAction)
                            }
                        }
                    }
                }
