...
Item {
    ...
    component WifiBody: Item {
        ...
        readonly property string selectedAdapterLabel: {
            if (!adapters || adapters.length === 0)
                return I18n.tr("Inbuilt Wifi");
            var cur = adapters.find(function(a) { return a.iface === wifi.selectedIface; });
            return cur ? cur.label : (adapters[0] ? adapters[0].label : I18n.tr("Inbuilt Wifi"));
        }

        readonly property var devices: (typeof Networking !== "undefined" && Networking && Networking.devices) ? Networking.devices.values : []
        readonly property var wifiDev: {
            if (wifi.selectedIface && wifi.selectedIface.length > 0) {
                var found = devices.find(function(d) {
                    return d && d.type === DeviceType.Wifi && (d.name === wifi.selectedIface || d.interface === wifi.selectedIface);
                });
                if (found) return found;
            }
            return devices.find(function(d) { return d && d.type === DeviceType.Wifi }) || null;
        }
        ...
        function initSelectedAdapter() {
            if (!wifi.adapters || wifi.adapters.length === 0)
                return;
            var found = wifi.adapters.find(function(a) { return a.iface === wifi.selectedIface; });
            if (!found) {
                var usb = wifi.adapters.find(function(a) { return a.isUsb; });
                if (usb) {
                    wifi.selectedIface = usb.iface;
                } else {
                    wifi.selectedIface = wifi.adapters[0].iface;
                }
            }
        }

        function selectAdapter(iface) {
            if (!iface || iface.length === 0 || wifi.selectedIface === iface)
                return;
            wifi.selectedIface = iface;
            wifi.expandedSsid = "";
            wifi.connecting = false;
            wifi.connectFailed = false;

            var cmd = "nmcli device set " + iface + " autoconnect yes 2>/dev/null || true; ";
            for (var i = 0; i < wifi.adapters.length; i++) {
                var other = wifi.adapters[i].iface;
                if (other !== iface) {
                    cmd += "nmcli device disconnect " + other + " 2>/dev/null || true; ";
                    cmd += "nmcli device set " + other + " autoconnect no 2>/dev/null || true; ";
                }
            }
            enforceProc.command = ["sh", "-c", cmd];
            enforceProc.running = true;
        }

        function refresh() {
            adapterProc.running = true;
            var iface = wifi.selectedIface;
            if (iface && iface.length > 0) {
                secProc.command = ["nmcli", "-t", "-f", "SSID,SECURITY,FREQ,BSSID", "dev", "wifi", "list", "ifname", iface];
            } else {
                secProc.command = ["nmcli", "-t", "-f", "SSID,SECURITY,FREQ,BSSID", "dev", "wifi", "list"];
            }
        ...
        function activateNetwork(net) {
            ...
            if (net.connected) {
                if (typeof net.disconnect === "function")
                    net.disconnect();
                else if (wifi.selectedIface.length) {
                    cleanupProc.command = ["nmcli", "device", "disconnect", wifi.selectedIface];
        ...
        function connectWithPassword(ssid, pw) {
            ...
            var bssid = wifi.pickedBssid(ssid);
            var target = bssid.length ? bssid : ssid;
            var iface = wifi.selectedIface;
            if (iface && iface.length > 0) {
                connProc.command = ["nmcli", "--ask", "dev", "wifi", "connect", target, "ifname", iface];
            } else {
                connProc.command = ["nmcli", "--ask", "dev", "wifi", "connect", target];
            }
        ...
        function connectBand(ssid, bssid) {
            ...
            var iface = wifi.selectedIface;
            if (iface && iface.length > 0) {
                connProc.command = ["nmcli", "dev", "wifi", "connect", bssid, "ifname", iface];
            } else {
        ...
        function startScan() {
            ...
            var iface = wifi.selectedIface;
            if (iface && iface.length > 0) {
                rescanProc.command = ["nmcli", "dev", "wifi", "rescan", "ifname", iface];
            } else {
                rescanProc.command = ["nmcli", "dev", "wifi", "rescan"];
            }
        ...
        Process {
            id: adapterProc
            command: ["sh", "-c", "usb=0; for i in $(ls -1 /sys/class/net 2>/dev/null); do if [ -d \"/sys/class/net/$i/wireless\" ] || [ -d \"/sys/class/net/$i/phy80211\" ]; then p=$(readlink -f \"/sys/class/net/$i/device\" 2>/dev/null || true); if echo \"$p\" | grep -q \"/usb\"; then usb=$((usb+1)); echo \"$i:USB Wifi-$usb:1\"; else echo \"$i:Inbuilt Wifi:0\"; fi; fi; done"]
        ...
        Item {
            ...
            // master on/off + adapter switcher.
            Item {
                id: wifiToggleRow
                z: 10
                ...
                Row {
                    ...
                    // Arrow / Adapter Picker button
                    Rectangle {
                        id: adapterBtn
                        anchors.verticalCenter: parent.verticalCenter
                        implicitHeight: 24
                        implicitWidth: adapterBtnRow.implicitWidth + 16
                        radius: Tokens.radius
                        color: wifi.adapterMenuOpen ? Tokens.tint10 : (adapterBtnH.hovered ? Tokens.tint5 : "transparent")
                        border.width: Tokens.border
                        border.color: wifi.adapterMenuOpen ? Tokens.lineStrong : (adapterBtnH.hovered ? Tokens.lineStrong : Tokens.line)
                        Behavior on color { ColorAnimation { duration: Tokens.snap } }
                        Behavior on border.color { ColorAnimation { duration: Tokens.snap } }
                        ...
                        Row {
                            ...
                            Text {
                                anchors.verticalCenter: parent.verticalCenter
                                text: wifi.selectedAdapterLabel
                                color: Tokens.ink
                                font.family: Tokens.ui
                                font.pixelSize: Tokens.fMicro
                                font.weight: Font.Medium
                            ...
                            Text {
                                anchors.verticalCenter: parent.verticalCenter
                                text: wifi.adapterMenuOpen ? "\u25b4" : "\u25be" // ▲ / ▾
                                color: Tokens.inkMuted
                                font.pixelSize: 10
                            }
                        }

                        HoverHandler { id: adapterBtnH; cursorShape: Qt.PointingHandCursor }
                        TapHandler { onTapped: wifi.adapterMenuOpen = !wifi.adapterMenuOpen }
                    }
                }
            ...
            // dismiss backdrop for adapter dropdown
            Item {
                anchors.fill: parent
                z: 150
                visible: wifi.adapterMenuOpen
                TapHandler { onTapped: wifi.adapterMenuOpen = false }
            }

            // adapter picker dropdown popover
            Rectangle {
                id: adapterDropdown
                z: 200
                visible: wifi.adapterMenuOpen && wifi.adapters.length > 0
                ...
                Column {
                    ...
                    Text {
                        text: I18n.tr("SELECT WI-FI ADAPTER")
                    ...
                    Repeater {
                        ...
                        delegate: Rectangle {
                            ...
                            height: 38
                            radius: Tokens.radius
                            readonly property bool isCurrent: modelData.iface === wifi.selectedIface
                            color: isCurrent ? Tokens.bone : (adH.hovered ? Tokens.tint10 : "transparent")
                            Behavior on color { ColorAnimation { duration: Tokens.snap } }
                            ...
                            Row {
                                ...
                                Column {
                                    ...
                                    Text {
                                        ...
                                        color: adItem.isCurrent ? Tokens.inkOnBone : Tokens.ink
                                        font.family: Tokens.ui
                                        font.pixelSize: Tokens.fSmall
                                        font.weight: adItem.isCurrent ? Font.DemiBold : Font.Medium
                                    }

                                    Text {
                                        text: modelData.iface + (modelData.isUsb ? " \u00b7 USB" : " \u00b7 Inbuilt (PCI/CNVi)")
                                        color: adItem.isCurrent ? Tokens.inkOnBoneDim : Tokens.inkMuted
                            ...
                            HoverHandler { id: adH; cursorShape: Qt.PointingHandCursor }
                            TapHandler {
                                onTapped: {
                                    wifi.selectAdapter(adItem.modelData.iface);
                                    wifi.adapterMenuOpen = false;
                                }
                            }
                        }
                    }
